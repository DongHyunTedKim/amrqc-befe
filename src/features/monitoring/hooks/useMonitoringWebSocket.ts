...
/**
 * 모니터링 페이지용 WebSocket 연결을 관리하는 커스텀 훅
 */
export function useMonitoringWebSocket(options: WebSocketOptions = {}) {
  ...
  const handleMessage = useCallback(
    (event: MessageEvent) => {
      try {
        ...
        switch (message.type) {
          case "welcome":
            console.log("Monitoring WebSocket 연결 성공:", message.data);
            break;
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onopen = () => {
        console.log("Monitoring WebSocket 연결됨");
        setState((prev) => ({
          ...prev,
          connected: true,
          reconnecting: false,
      ...
      ws.onerror = (error) => {
        console.error("Monitoring WebSocket 오류:", error);
        setState((prev) => ({
          ...prev,
      ...
      ws.onclose = (event) => {
        ...
        if (
          ...
          console.log(
            `Monitoring 재연결 시도 ${reconnectAttemptsRef.current}/${opts.maxReconnectAttempts}`
          );
      ...
      console.error("Monitoring WebSocket 연결 실패:", error);
      setState((prev) => ({
        ...prev,
        connected: false,
  ...
  // 자동 연결 - wsUrl이 변경될 때만 연결
  useEffect(() => {
    if (opts.autoConnect && wsUrl && !wsRef.current) {
      connect();
    }
